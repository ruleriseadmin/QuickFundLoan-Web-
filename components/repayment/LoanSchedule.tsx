    ...
    <div key={index} className='grid lg:gap-2 md:gap-2  text-[16px] grid-cols-2 align-middle lg:px-4 md:px-4 px-2 pt-8 '>
      ...
      <div className={`${loan.status === 'completed' ? 'text-[#5A5A5A]' : 'text-[#282828] '} text-[16px]  leading-7 flex  flex-col`}>
        <div className='flex flex-col justify-end items-end'>
         ...
         loanHistory[0]?.has_upfront_interest ?
        loanHistory[0]?.status === 'OVERDUE' ? formatCurrency(loan.remaining_balance + loanHistory[0]?.penalty_remaining) : formatCurrency(loan?.principal_payment)
        : loanHistory[0]?.status === 'OVERDUE' ? formatCurrency(loan.remaining_balance + loanHistory[0]?.penalty_remaining) : formatCurrency(loan?.payment_amount)}</span>
