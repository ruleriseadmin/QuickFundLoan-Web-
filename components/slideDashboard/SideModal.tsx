...
const SideModal: React.FC<SideModalProps> = ({ isOpen, closeModal }) => {
  ...
  const monoSuccess = localStorage.getItem('monoSuccess');
...
const showLinkAccount = () => {
  ...
  useEffect(() => {
    ...
    if (onboarding && onboarding?.bank_account_exists === 'completed' && monoSuccess)
    {
      localStorage.removeItem('monoSuccess')
    }
  ...
  }, [bankAccounts, onboarding, monoSuccess]);
...
useEffect(() => {
  const handleOnboarding = async () => {
    ...
    if ( !onboarding?.profile_exists) {
      ...
      await router.push(
        '/dashboard?message=Dear customer, your bank account verification is in progress. We will notify you shortly once it is done.'
      );
    } else if (onboarding?.bank_account_exists === 'pending'  && !monoSuccess) {
      await showLinkAccount();
    }else if(!onboarding.bvn_nin_exists) {
      showVerifyIdentity();
  ...
  handleOnboarding();
}, [initialization, onboarding, bankAccounts, monoSuccess, debitMessage, router]);
